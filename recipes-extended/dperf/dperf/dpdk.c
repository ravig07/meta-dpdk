...
static int dpdk_eal_init(struct config *cfg, char *argv0, char *prefix_name[])
{
    int argc = 4;
    ...
    if(strcmp(prefix_name[0], "no")!=0){
    	argv[argc] = "--file-prefix";
    	argv[argc+1] = prefix_name[0];
    	printf("File_prefix name is %s \n", prefix_name[0]);
    	argc+=2;
    }
    else {
    	printf("no flag \n");
...
int dpdk_init(struct config *cfg, char *argv0, char *prefix_name[])
{
    if (dpdk_eal_init(cfg, argv0, prefix_name) < 0) {
